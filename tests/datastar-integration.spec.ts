...
test.describe("Datastar Integration Tests", () => {
  ...
  test("Test 1: Home Page - Load and Verify Datastar Script", async ({
    page,
  }) => {
    ...
    const hasDatastarScript =
      content.includes("@starfederation/datastar") ||
  ...
  test("Test 2: Check Datastar Script Element in DOM", async ({ page }) => {
    ...
    const datastarScript = await page.evaluate(() => {
      ...
      const datastarScripts = scripts.filter(
        (script) =>
    ...
    console.log(
      "Datastar scripts found:",
      JSON.stringify(datastarScript, null, 2),
    );
  ...
  test("Test 6: Network Analysis - Check Datastar CDN Request", async ({
    page,
  }) => {
    ...
    page.on("request", (request) => {
      if (
        request.url().includes("datastar") ||
        request.url().includes("starfederation")
      ) {
    ...
    page.on("response", (response) => {
      if (
        ...
        response.url().includes("starfederation")
      ) {
    ...
    console.log(
      "Datastar network requests:",
      JSON.stringify(datastarRequests, null, 2),
    );
